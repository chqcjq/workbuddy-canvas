...
5. Generate a new bitmap with the Cowart MCP image API tools.

   Preferred provider:

   - Use `create_nano_banana_image` with `imageUrls` when the selected holder/source metadata has `meta.cowartImageProvider: "banana"`, or when no provider is specified.
   - Use `create_gpt_image_2` with `images` when the selected holder/source metadata has `meta.cowartImageProvider: "image2"`, or when the user explicitly asks for `gpt-image-2` / Image2.
   ...
   NANO-BANANA image-edit call shape:

   ```json
   {
     "prompt": "<clean edit prompt>",
     "imageUrls": ["<publicly reachable source or annotation image URL>"],
   ...
   The Duomi NANO-BANANA edit API accepts reference images only as URLs. It does not accept local file paths or base64. If the user only supplied a local screenshot and no reachable image URL is available, ask for a hosted image URL or use another available image-generation flow that can consume local images.
   ...
   gpt-image-2 image-to-image call shape:

   ```json
   {
     "prompt": "<clean edit prompt>",
     "images": ["<publicly reachable source or annotation image URL>"],
     "size": "16:9",
     "oversea": false
   }
   ```
...
6. Insert the revised image beside the original with Cowart MCP.
   ...
   If the selected source/holder has provider metadata, honor it:

   ```text
   meta.cowartImageProvider: "banana" | "image2"
   ```

   `banana` means use NANO-BANANA image editing. `image2` means use `gpt-image-2` image-to-image. If the metadata is missing, default to `banana`.
