...
const TOOL_CREATE_NANO_BANANA_IMAGE = "create_nano_banana_image";
const TOOL_GET_NANO_BANANA_TASK = "get_nano_banana_task";
const TOOL_CREATE_GPT_IMAGE_2 = "create_gpt_image_2";
const TOOL_GET_IMAGE_TASK = "get_image_task";
const TOOL_DOWNLOAD_IMAGE = "download_cowart_image";
const PAGE_ID_PREFIX = "page:";
const PAGE_ASSETS_ROUTE = "/page-assets/";
const CANVAS_FILE_NAME = "cowart-canvas.json";
const PRIMARY_IMAGE_API_BASE_URL = "https://duomiapi.com";
const FALLBACK_IMAGE_API_BASE_URL = "https://api.wike.cc";
...
async function readSelectionState(args) {
  ...
  try {
    const selection = JSON.parse((await readFile(selectionFile, "utf8")).replace(/^\uFEFF/, ""));
...
async function resolveImageApiConfig(args = {}) {
  ...
  const apiBaseUrl =
    nonEmptyString(args.apiBaseUrl) ||
    nonEmptyString(args.imageApi?.apiBaseUrl) ||
    nonEmptyString(selectionImageApi.apiBaseUrl) ||
    nonEmptyString(process.env.COWART_IMAGE_API_BASE_URL) ||
    nonEmptyString(process.env.DUOMI_API_BASE_URL) ||
    PRIMARY_IMAGE_API_BASE_URL;
  const apiKey =
    nonEmptyString(args.apiKey) ||
    nonEmptyString(args.imageApi?.apiKey) ||
    nonEmptyString(selectionImageApi.apiKey) ||
    nonEmptyString(process.env.COWART_IMAGE_API_KEY) ||
    nonEmptyString(process.env.DUOMI_API_KEY) ||
    nonEmptyString(process.env.WIKE_API_KEY);
...
async function fetchJsonWithFallback(path, args = {}, options = {}) {
  const { apiBaseUrl, apiKey } = await resolveImageApiConfig(args);
  if (!apiKey) throw new Error("Image API key is required. Set DUOMI_API_KEY or COWART_IMAGE_API_KEY, or pass apiKey.");

  const preferredBaseUrl = apiBaseUrl;
  const baseUrls = args.useFallback === false ? [preferredBaseUrl] : [...new Set([preferredBaseUrl, FALLBACK_IMAGE_API_BASE_URL])];
  const headers = {
  ...
  for (const baseUrl of baseUrls) {
    try {
      const payload = await fetchJson(`${baseUrl}${path}`, { ...options, headers });
      return { baseUrl, payload };
    } catch (error) {
      lastError = error;
      if (args.useFallback === false) break;
    }
  }
...
function normalizeImageTask(kind, baseUrl, payload) {
  const data = payload?.data;
  const taskPayload = data && typeof data === "object" ? data : payload;
  const nestedData = taskPayload?.data && typeof taskPayload.data === "object" ? taskPayload.data : null;
  const images = Array.isArray(nestedData?.images)
    ? nestedData.images
    : Array.isArray(taskPayload?.images)
      ? taskPayload.images
  ...
  return {
    kind,
    baseUrl,
    id: payload?.id ?? data?.task_id ?? taskPayload?.task_id ?? taskPayload?.id ?? null,
    state: taskPayload?.state ?? payload?.state ?? null,
    progress: taskPayload?.progress ?? payload?.progress ?? null,
    message: taskPayload?.msg ?? payload?.msg ?? null,
    description: nestedData?.description ?? taskPayload?.description ?? null,
    images,
    raw: payload,
  };
}
...
async function createNanoBananaImage(args = {}) {
  const prompt = nonEmptyString(args.prompt);
  if (!prompt) throw new Error("prompt is required.");

  const imageUrls = Array.isArray(args.imageUrls)
    ? args.imageUrls.map(nonEmptyString).filter(Boolean)
    : Array.isArray(args.image_urls)
      ? args.image_urls.map(nonEmptyString).filter(Boolean)
      : [];
  const isEdit = imageUrls.length > 0 || args.mode === "edit";
  const body = compactObject({
    model: nonEmptyString(args.model) || "gemini-3-pro-image-preview",
    prompt,
    aspect_ratio: nonEmptyString(args.aspectRatio) ?? nonEmptyString(args.aspect_ratio) ?? "auto",
    image_size: nonEmptyString(args.imageSize) ?? nonEmptyString(args.image_size) ?? "1K",
    oversea: typeof args.oversea === "boolean" ? args.oversea : false,
    ...(isEdit ? { image_urls: imageUrls } : {}),
  });
  const path = isEdit ? "/api/gemini/nano-banana-edit" : "/api/gemini/nano-banana";
  const { baseUrl, payload } = await fetchJsonWithFallback(path, args, {
    method: "POST",
    body: JSON.stringify(body),
  });

  return normalizeImageTask(isEdit ? "nano-banana-edit" : "nano-banana", baseUrl, payload);
}

async function getNanoBananaTask(args = {}) {
  const id = nonEmptyString(args.id) || nonEmptyString(args.taskId) || nonEmptyString(args.task_id);
  if (!id) throw new Error("id is required.");

  const { baseUrl, payload } = await fetchJsonWithFallback(`/api/gemini/nano-banana/${encodeURIComponent(id)}`, args);
  return normalizeImageTask("nano-banana", baseUrl, payload);
}

async function createGptImage2(args = {}) {
  const prompt = nonEmptyString(args.prompt);
  if (!prompt) throw new Error("prompt is required.");

  const images = Array.isArray(args.images)
    ? args.images.map(nonEmptyString).filter(Boolean)
    : Array.isArray(args.image)
      ? args.image.map(nonEmptyString).filter(Boolean)
      : [];
  const body = compactObject({
    model: nonEmptyString(args.model) || "gpt-image-2",
    prompt,
    size: nonEmptyString(args.size) || "16:9",
    oversea: typeof args.oversea === "boolean" ? args.oversea : false,
    ...(images.length > 0 ? { image: images } : {}),
  });
  const { baseUrl, payload } = await fetchJsonWithFallback("/v1/images/generations?async=true", args, {
    method: "POST",
    body: JSON.stringify(body),
  });

  return normalizeImageTask("gpt-image-2", baseUrl, payload);
}

async function getImageTask(args = {}) {
  const id = nonEmptyString(args.id) || nonEmptyString(args.taskId);
  if (!id) throw new Error("id is required.");

  const { baseUrl, payload } = await fetchJsonWithFallback(`/v1/tasks/${encodeURIComponent(id)}`, args);
  return normalizeImageTask("image-task", baseUrl, payload);
}

async function downloadCowartImage(args = {}) {
  const imageUrl = nonEmptyString(args.url) || nonEmptyString(args.imageUrl);
  if (!imageUrl) throw new Error("url is required.");

  const outputDir = pathResolve(nonEmptyString(args.outputDir) || join(process.cwd(), "cowart-generated-images"));
  const requestedName = sanitizeFileName(args.fileName || basename(new URL(imageUrl).pathname) || "image.png");
  const { filePath, fileName } = await uniqueFilePath(outputDir, requestedName);
...
function toolDefinitions() {
  return [
    ...
    {
      name: TOOL_CREATE_NANO_BANANA_IMAGE,
      title: "Create NANO-BANANA Image",
      description:
        "Submit a NANO-BANANA text-to-image or image-edit task to the Duomi image API. Returns an async task id; call get_nano_banana_task to poll it.",
      inputSchema: {
        type: "object",
        properties: {
          ...
          imageUrls: {
            type: "array",
            items: { type: "string" },
            description: "Reference image URLs. Provide one or more URLs for image editing; omit for text-to-image.",
          },
          model: {
            type: "string",
            enum: ["gemini-3-pro-image-preview", "gemini-2.5-flash-image", "gemini-3.1-flash-image-preview"],
            description: "Defaults to gemini-3-pro-image-preview.",
          },
          aspectRatio: {
            type: "string",
          ...
          imageSize: { type: "string", enum: ["1K", "2K", "4K"], description: "Defaults to 1K." },
          oversea: { type: "boolean", description: "Duomi oversea flag. Defaults to false." },
          projectDir: { type: "string", description: "Absolute Cowart project directory. Used to read canvas API config." },
          canvasDir: { type: "string", description: "Absolute canvas directory. Overrides projectDir for reading API config." },
          apiKey: { type: "string", description: "Optional API key. Prefer DUOMI_API_KEY or COWART_IMAGE_API_KEY env vars." },
          apiBaseUrl: { type: "string", description: "Optional API base URL. Defaults to https://duomiapi.com." },
          useFallback: { type: "boolean", description: "Whether to retry https://api.wike.cc after the primary base URL. Defaults to true." },
        },
        required: ["prompt"],
        additionalProperties: false,
      },
    ...
    {
      name: TOOL_GET_NANO_BANANA_TASK,
      title: "Get NANO-BANANA Task",
      description:
        "Poll a NANO-BANANA task from the Duomi image API. Succeeded tasks include image URLs under structuredContent.images.",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "string", description: "Task id returned by create_nano_banana_image." },
          projectDir: { type: "string", description: "Absolute Cowart project directory. Used to read canvas API config." },
          canvasDir: { type: "string", description: "Absolute canvas directory. Overrides projectDir for reading API config." },
          apiKey: { type: "string", description: "Optional API key. Prefer DUOMI_API_KEY or COWART_IMAGE_API_KEY env vars." },
          apiBaseUrl: { type: "string", description: "Optional API base URL. Defaults to https://duomiapi.com." },
    ...
    {
      name: TOOL_CREATE_GPT_IMAGE_2,
      title: "Create GPT Image 2",
      description:
        "Submit a gpt-image-2 async text-to-image or image-to-image task to /v1/images/generations?async=true. Omit images for text-to-image; pass images for image-to-image. Returns an id; call get_image_task to poll it.",
      inputSchema: {
        type: "object",
        properties: {
          prompt: { type: "string", description: "Image prompt." },
          size: { type: "string", description: "Image size or aspect ratio, for example 1:1, 16:9, 9:16." },
          images: {
            type: "array",
            items: { type: "string" },
            description: "Optional reference image URLs for image-to-image generation. Omit for text-to-image.",
          },
          oversea: { type: "boolean", description: "Duomi oversea flag. Defaults to false." },
          projectDir: { type: "string", description: "Absolute Cowart project directory. Used to read canvas API config." },
          canvasDir: { type: "string", description: "Absolute canvas directory. Overrides projectDir for reading API config." },
    ...
    {
      name: TOOL_GET_IMAGE_TASK,
      title: "Get Image Task",
      description:
        "Poll a generic Duomi /v1/tasks/{id} async image task. Succeeded tasks include image URLs under structuredContent.images.",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "string", description: "Task id returned by create_gpt_image_2." },
          projectDir: { type: "string", description: "Absolute Cowart project directory. Used to read canvas API config." },
          canvasDir: { type: "string", description: "Absolute canvas directory. Overrides projectDir for reading API config." },
          apiKey: { type: "string", description: "Optional API key. Prefer DUOMI_API_KEY or COWART_IMAGE_API_KEY env vars." },
          apiBaseUrl: { type: "string", description: "Optional API base URL. Defaults to https://duomiapi.com." },
    ...
    },
    {
      ...
      title: "Download Cowart Image",
      description:
        "Download a generated image URL to a local file so it can be inserted with insert_cowart_image.",
      inputSchema: {
        type: "object",
        properties: {
          url: { type: "string", description: "Generated image URL." },
          outputDir: { type: "string", description: "Local output directory. Defaults to ./cowart-generated-images." },
...
async function handleToolCall(id, params) {
    ...
    const summary =
      selectedShapes.length === 0
        ...
        : selectedShapes
            ...
            .join("\n");
    const imageApiSummary = selection.imageApi
      ? `\nImage API: ${selection.imageApi.apiBaseUrl ?? PRIMARY_IMAGE_API_BASE_URL}; key ${
          selection.imageApi.hasApiKey || nonEmptyString(selection.imageApi.apiKey) ? "configured" : "missing"
        }.`
      : "\nImage API: not configured on canvas.";

    sendResult(id, {
      content: [{ type: "text", text: `${summary}${imageApiSummary}` }],
  ...
  if (params?.name === TOOL_CREATE_NANO_BANANA_IMAGE) {
    const result = await createNanoBananaImage(params.arguments ?? {});
    sendResult(id, {
      content: [{ type: "text", text: `Submitted ${result.kind} task ${result.id ?? "(missing id)"} via ${result.baseUrl}.` }],
      structuredContent: result,
    });
    return;
  }

  if (params?.name === TOOL_GET_NANO_BANANA_TASK) {
    const result = await getNanoBananaTask(params.arguments ?? {});
    sendResult(id, {
      content: [
        {
          type: "text",
          text: `NANO-BANANA task ${result.id ?? params.arguments?.id} is ${result.state ?? "unknown"} with ${result.images.length} image(s).`,
        },
      ],
      structuredContent: result,
    });
  ...
  if (params?.name === TOOL_CREATE_GPT_IMAGE_2) {
    const result = await createGptImage2(params.arguments ?? {});
    sendResult(id, {
      content: [{ type: "text", text: `Submitted gpt-image-2 task ${result.id ?? "(missing id)"} via ${result.baseUrl}.` }],
      structuredContent: result,
    });
    return;
  }

  if (params?.name === TOOL_GET_IMAGE_TASK) {
    const result = await getImageTask(params.arguments ?? {});
    sendResult(id, {
      content: [
        {
          type: "text",
          text: `Image task ${result.id ?? params.arguments?.id} is ${result.state ?? "unknown"} with ${result.images.length} image(s).`,
        },
      ],
      structuredContent: result,
    });
  ...
  if (params?.name === TOOL_DOWNLOAD_IMAGE) {
    const result = await downloadCowartImage(params.arguments ?? {});
    sendResult(id, {
      content: [{ type: "text", text: `Downloaded ${result.fileName} (${result.byteLength} bytes) to ${result.filePath}.` }],
      structuredContent: result,
    });
    return;
  }
